...
from app.pipeline.job_runner import has_running_job_for_book, start_job
...
def _create_and_start_job(book_id: str, source_lang: str) -> JobOut:
...
async def translate_book(book_id: str) -> JobOut:
    ...
    if has_running_job_for_book(book_id):
        raise HTTPException(status_code=409, detail="A translation job is already running for this book")
    return _create_and_start_job(book_id, book["source_lang"])
